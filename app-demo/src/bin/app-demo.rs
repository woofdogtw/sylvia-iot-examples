...
use app_demo::{libs, routes};

#[derive(Deserialize)]
struct AppConfig {
    log: logger::Config,
    server: server_config::Config,
    #[serde(rename = "appDemo")]
    app_demo: libs::config::Config,
}

const PROJ_NAME: &'static str = env!("CARGO_PKG_NAME");
const PROJ_VER: &'static str = env!("CARGO_PKG_VERSION");
const HTTP_PORT: u16 = 7080;
const HTTPS_PORT: u16 = 7443;
...
async fn main() -> std::io::Result<()> {
    ...
    let app_demo_state = match routes::new_state("/app-demo", &conf.app_demo).await {
        Err(e) => {
    ...
    let mut serv = HttpServer::new(move || {
        ...
        App::new()
            ...
            .service(routes::new_service(&app_demo_state))
...
fn init_config() -> Result<AppConfig, Box<dyn StdError>> {
    ...
    Ok(AppConfig {
        ...
        app_demo: libs::config::read_args(&args),
    })
}
